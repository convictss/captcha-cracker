 ...
 * @description: selenium识别极验滑动验证码(canvas)，下载图片方式为base64转图片
 *
 * 1.下载缺口图、完整图、全页面图
 * 2.比较缺口图、完整图，获得移动距离
 * 3.根据移动距离，模拟真人鼠标移动
 */
public class GeetestCanvasByBase64 {
    ...
    public static void main(String[] args) {
        ...
        try {
            for (int i = 0; i < showCount; i++) {
                ...
                // 下载完整图
                String fullJS = "var base64Obj =document.getElementsByClassName('geetest_canvas_fullbg geetest_fade geetest_absolute')[0];return base64Obj.toDataURL();";
                String fullBase64Str = jsExecutor.executeScript(fullJS).toString();
                byte[] fullBytes = base64Decoder.decodeBuffer(fullBase64Str.replace("data:image/png;base64,", ""));
                BufferedImage fullBI = ImageIO.read(new ByteArrayInputStream(fullBytes));
                ImageIO.write(fullBI, "png", new File(BASE_PATH + FULL_IMAGE_NAME));

                // 下载缺口图
                String cutJS =  "var base64Obj =document.getElementsByClassName('geetest_canvas_bg geetest_absolute')[0];return base64Obj.toDataURL();";
                String cutBase64Str = jsExecutor.executeScript(cutJS).toString();
                byte[] cutBytes = base64Decoder.decodeBuffer(cutBase64Str.replace("data:image/png;base64,", ""));
                BufferedImage cutBI = ImageIO.read(new ByteArrayInputStream(cutBytes));
